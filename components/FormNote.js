  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col min-h-vh90 justify-center items-center gap-4">
      ...
      <div className="flex flex-col items-start">
        ...
        <input type="text" placeholder={title} defaultValue={title} onChange={(e) => setTitle(e.target.value)} className="focus:outline-none rounded w-80 h-8 pl-1 border-2 border-black" />
      </div>
      <div className="flex flex-col items-start">
        <label className="font-bold">Conteúdo:</label>
        <textarea defaultValue={content} onChange={(e) => setContent(e.target.value)} className="focus:outline-none rounded w-80 pl-1 border-2 border-black" />
      </div>
